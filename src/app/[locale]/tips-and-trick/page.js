    ...
    <div className="border-b border-slate-200/80 last:border-b-0">
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className={`text-teaser mt-4 max-w-xl transition-all duration-500 ${isOpen ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
            {tip.teaser}
          </p>
        </div>
      ...
      <div className={`grid transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
        <div className="overflow-hidden">
          <div className="pl-[3.25rem] md:pl-[4.25rem] pb-7 pr-4 max-w-3xl">
            <p>{tip.body}</p>
            <p className="text-takeaway uppercase tracking-widest mt-6" style={{ color: accent }}>
              {tip.takeaway}
            </p>
          </div>
        </div>
  ...
  return (
    <div className={`flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} gap-12 md:gap-16 items-start`}>
      ...
      <div className="w-full md:w-3/5">
        <h3 className="mb-6" style={{ color: cat.accent }}>
          {String(idx + 1).padStart(2, '0')}. {cat.label}
        </h3>
        
        <h3 className="text-brand-navy text-balance">
          {cat.intro}
        </h3>

        <div className="mt-16 border-t border-slate-200/80">
          {cat.tips.map((tip, i) => {
            const key = `${cat.key}-${i}`;
            return (
              <TipRow
